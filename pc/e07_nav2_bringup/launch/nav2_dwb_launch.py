...
def generate_launch_description():
    ...
    params_file = LaunchConfiguration('params_file', default=os.path.join(bringup_dir, 'params', 'nav2_teb_params.yaml'))
    #### ✅ params_file 是你希望加载的文件
    ...
    # 声明参数文件参数
    # ❌ 实际 launch 时会使用这个文件（覆盖了前面那个）
    declare_params_file_cmd = DeclareLaunchArgument(
        'params_file',
        default_value=os.path.join(bringup_dir, 'params', 'nav2_dwb_params.yaml'),
        #修改为如下：
        #default_value=os.path.join(bringup_dir, 'params', 'nav2_teb_params.yaml'),
    ...
    load_nodes = GroupAction(
        ...
        actions=[
            Node(
                ...
                arguments=['--ros-args', '--log-level', 'nav2_custom_controller:=info'],
